...
class Students extends CI_Controller {
	...
	public function new() {
		...
		$this->load->view('templates/navigation',$nav_items);
		$data['clearance_options'] = $this->Clearance_Model->get_clearance_levels();
		$this->load->view('students/Student_add_view', $data);
		$this->load->view('templates/footer');
	}
	...
	public function validation(){
		...
		$banner_id = $this->input->post("banner_id");
		$exists = $this->Student_Model->get_student($banner_id);

		if ($exists != null){
			// $status = $this->Student_Model->check_status($barcode);
			// $states = $this->Student_Model->get_status_enum("array");
			//
			// //available
			// if ($status == $states[0] || $status == $states[1]){
			// 	echo json_encode(array(true,$barcode, $available));
			// } else {
			// 	echo json_encode(array(false,$barcode, $available));
			// }
		} else {
			$message = "The student does not exist in the Checkout System. Please see Mary.";
